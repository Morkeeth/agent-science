#!/usr/bin/env python3
"""Baseline vs shipping: hosted partner surface.

Naive arm: anything with ok=true and service=agent-science passes.
Shipping arm: partner fields + public /partners JSON + /clear not public.

Against live revision 00026 (pre-fix), naive wins and shipping fails —
that is the finding. Against a local hosted-mode server with the fix, shipping
wins. Re-run after Oscar deploys this branch.

Usage:
  python3 scripts/eval_hosted_partner_surface.py
  python3 scripts/eval_hosted_partner_surface.py --local
"""
...
def shipping_score(health: dict, partners_code: int, partners: object, clear_code: int) -> bool:
    required = (
        "gemini",
        "gemini_path",
        "parallel",
    ...
    if "track_checklist" not in partners or "partners" not in partners:
        return False
    # Public /clear would break the workspace boundary.
    if clear_code not in (401, 303, 404, 405):
        return False
    return True
...
def score_target(base: str) -> dict:
    ...
    shipping = shipping_score(health, p_code, partners, c_code) if h_code == 200 else False
    return {
        "base": base,
        "health_http": h_code,
...
def main() -> int:
    ...
    print("shipping arm: partner fields + public /partners + /clear not public")
    print()
    for name, row in rows:
        print(f"[{name}] {row['base']}")
        print(f"  health={row['health_http']} partners={row['partners_http']} clear={row['clear_http']}")
    ...
    if live_naive and not live_ship:
        print(
            "FINDING: live hosted loses to naive liveness — partner fields absent "
            f"(revision={live['health'].get('revision')}). Oscar must deploy this branch."
        )
    elif live_ship:
        print("FINDING: live hosted passes shipping partner surface.")
    else:
    ...
    if loc_ship is False:
        ...
        return 1
    if loc_ship is True:
        print("LOCAL FIX: shipping arm passes on private-workspaces with partner fields restored.")
    return 0
