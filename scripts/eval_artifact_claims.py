...
def _measure_hosted_public_search():
    """Pack/Devpost still point judges at unauthenticated /search — measure it."""
...
def _claims() -> list[Claim]:
    ...
    pack_claims_private = bool(re.search(
        r"Private until submit|flip repo to public", pack, re.I))
    pack_claims_public_search = bool(re.search(
        r"GET /search|hosted.*SOURCED|/search\?q=", pack, re.I)) and not bool(
        re.search(r"private-workspaces|local-only|/search.*sign-in", pack, re.I))
    ...
    return [
        ...
        Claim("AC7", "hosted public /search SOURCED without auth",
              pack_claims_public_search, _measure_hosted_public_search()),
