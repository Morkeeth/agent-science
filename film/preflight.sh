...
ok "hosted /health (partner fields — fails on liveness-only revisions)"
...
assert not missing, 'liveness-only /health missing '+str(missing)+' — deploy partner-surface branch'
assert d.get('engine_default')=='adk', d
" || red "/health bad (partner fields or engine_default)"
