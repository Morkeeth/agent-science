...
**Stranger demo (after backfill):**
```bash
python3 clear_corpus.py research-corpus --backfill
python3 ask_registry.py "arxiv:2511.12884"    # → SOURCED + verbatim span
python3 ask_registry.py "1.8m adversarial"    # → UNSOURCED + named refusal
...
- [ ] Live full chain: `agent_science.py` on dust-bowl — **blocked on this VM:** no `~/.config/keys/{gemini,parallel}.key`, no Vertex ADC
...
### Controls
- `python3 tests/test_watch_it_go_red.py` → **71 passed, 1 failed** (`empty search` — no Parallel key, search not performed → `no_source_offered` not `search_found_nothing`)
