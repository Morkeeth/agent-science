...
- [x] `resolve_gemini_path` requires ADC token or API key — `docs/FINDING-gemini-health-env-alone-2026-09-20.md`
- [x] `/health` adds `gemini_configured` + receipt-backed `verified_search_id` fields
- [x] `track_checklist.parallel_search_at_runtime` ← key presence (not hardcoded True)
...
- [x] SUBMISSION-PACK — `bench_check_docs.py` **128/128** (superseded 2026-09-20 → **130/130**)
...
- **Intended user outcome:** a judge or Oscar deploy verify can trust `/health` partner
  ...
  receipt-backed search_ids survive cold start; PeriodCheck's published eval remains the
  external bar for live Parallel IDs.
...
- **Largest product gap:** live Cloud Run still serves stripped health until Oscar deploy;
  no Parallel key / workspace token on this VM for live `/clear`.
- **Next build change:** Oscar `deploy.sh` then `EXPECT_STATE=green` watch +
  `verify_partners_hosted.sh`; with token, compound or keep BLOCKED honestly.
