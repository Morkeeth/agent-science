...
## 🎯 NOW — Partner surfaces under private-workspaces (2026-09-13 night)

**Slice:** Hosted `/health` + `/partners` went dark under private-workspaces; restore judge-facing partner prove; local desk ADK path; docs match object.

### Build

- [x] Shared `cloud/partners.health_payload()` — used by local desk + hosted case_http
- [x] Public hosted `GET /health` (full partner fields) + `GET /partners` (no login)
- [x] `scripts/verify_partners_local.sh` — local desk prove (`engine_default: adk`)
- [x] `scripts/verify_partners_hosted.sh` — private-workspaces aware; `/clear` local-only
- [x] Partner doc + FINDING + RECEIPT + DESIGN-PARTNER-LOOP + promise line README
- [x] SUBMISSION-PACK counts re-derived — partner_runtime **8/8** · total **129/129**
- [x] Qwen eval gate re-run — baseline/ablation **5/6 vs 6/6**, delta +1
- [x] Live compound — honest BLOCKED (no Parallel/Gemini keys on this VM)
...
bash scripts/verify_partners_local.sh                                # local OK · engine_default=adk
...
# After Oscar deploy.sh of this commit:
bash scripts/verify_partners_hosted.sh                               # health + partners (clear SKIP)
```

### Receipt

- `docs/RECEIPT-partner-surfaces-2026-09-13.md`
- `docs/FINDING-hosted-partner-surfaces-2026-09-13.md`
- `docs/BLOCKED-live-compound-2026-09-13.md`

### BLOCKED

- Hosted partner verify green — **Oscar `deploy.sh`** required (rev `00028-hed` still minimal health)
- Live Parallel `/clear` on this VM — **PARALLEL_API_KEY** missing
- Orphan-works full script — prior **504** @ 300s
...
|------|------|---------|---------|
| 2026-09-13 night | Hosted partner surfaces RED → fix | `curl …/health` · `verify_partners_hosted.sh` · `verify_partners_local.sh` | Hosted health lacked partners (rev 00028); local `engine_default=adk`; tests 72/72 · partner 8/8 · bench 129/129 |
| 2026-09-13 night | Qwen eval re-run + live compound | `eval_refusal_baseline.py` · `eval_refusal_ablation.py` | baseline/ablation **5/6** vs shipping **6/6**; live compound **BLOCKED** (no keys) |
