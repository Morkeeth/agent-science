...
def resolve_gemini_path() -> str:
    """How Gemini answers on this process — never invent a key path."""
...
def health_payload(*, mode: str | None = None) -> dict:
    """Shared /health shape for local desk and hosted workspaces.

    Hosted private-workspaces used to return only {ok, service, mode, revision}.
    That passed liveness while partner fields were invisible — a false green for
    the Sep 9 partner gate. Partner fields are public; they reveal wiring, not
    tenant data.
    """
    path = resolve_gemini_path()
    adk_ok = adk_agent.adk_available()
    adk_on = adk_default_enabled() and adk_ok
    out = {
        "ok": True,
    ...
    if mode:
        out["mode"] = mode
    # Hosted boundary: unauthenticated /clear is local-desk only.
    if mode == "private-workspaces":
        out["clearance_desk"] = "local-only"
        out["hosted_parallel_path"] = "clearance/cases.py → find_sources"
        out["adk_clear_path"] = "local desk POST /clear (not hosted)"
    return out
...
def manifest(*, gemini_path: str | None = None, adk_default: bool | None = None) -> dict:
    ...
    hosted = os.getenv("AGENT_SCIENCE_HOSTED") == "1" or bool(os.getenv("K_SERVICE"))
    return {
        ...
        "partners": {
            ...
            "parallel": {
                **parallel_search.integration_info(),
                ...
                "called_from": (
                    "clearance/cases.py → find_sources (hosted research); "
                    "clearance/facts.py → judge_claim (local /clear)"
                ),
            },
            "google_cloud": {
                "role": "Cloud Run private workspaces + local clearance desk",
                "module": "cloud/service.py",
                "deploy": "deploy.sh",
                "hosted_mode": "private-workspaces" if hosted else "local-desk",
            ...
            "agent_builder_adk": {
                "role": "default /clear engine on local clearance desk",
                ...
                "hosted_note": (
                    "Hosted Cloud Run serves private workspaces; ADK /clear runs on "
                    "local desk (AGENT_SCIENCE_HOSTED unset)."
                ),
            },
        },
        "track_checklist": {
            ...
            "unauthenticated_clear_local_only": True,
        },
        "receipts": [
