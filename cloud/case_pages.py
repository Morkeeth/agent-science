...
def dashboard(case_list, csrf, budget, error=''):
    ...
    body += ('<div class="layout"><section class="panel stamp"><p class="eyebrow">01 / Start a case</p><h2>Give the question a place to live.</h2>'
             ...
             '<aside><section class="panel dark"><p class="eyebrow" style="color:#d6aaa4">A useful return visit</p><h2>The decision stays yours.</h2>'
...
def detail(case, csrf, error=''):
    ...
    for d in decisions:
        ...
        body += (f'<article class="panel{ " stamp" if alert else ""}"><span class="badge{ " alert" if alert else ""}">{"REVIEW REQUIRED" if alert else "Unchanged in saved snapshots"}</span>'
        ...
        body += '</p>' + (render_changes(review.get('changes', []), nested=True) if alert else '<p class="quiet">No change in the compared saved evidence. This does not establish that the web is unchanged.</p>') + '</article>'
    if not decisions:
    ...
    if usable and version == latest:
        body += (f'<section class="panel stamp"><h3>Record a decision</h3><p class="small">State your choice and explain how the cited evidence informs it. Saving does not mark the evidence as proof.</p>'
                 ...
                 '<label for="statement">What will you do?</label><textarea id="statement" name="statement" rows="2" required maxlength="4000"></textarea>'
