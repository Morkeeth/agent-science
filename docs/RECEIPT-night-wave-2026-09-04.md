...
**Slice:** Artifact-claims falsifiable gate + SUBMISSION-PACK/STATUS truth + null arm + blocked live compound + deploy prep  
**Branch:** `cursor/night-wave-artifact-claims-acb8`  
**Keys on VM:** PARALLEL missing · GEMINI missing · ADC missing → live compound **BLOCKED**
...
7. **Live compound BLOCKED** — `docs/BLOCKED-live-compound-exhibit-2026-09-04.md`
...
**Live compound exhibit:**

```bash
test -n "${PARALLEL_API_KEY:-}" || test -f ~/.config/keys/parallel.key  # missing
test -n "${GEMINI_API_KEY:-}" || test -n "${GOOGLE_APPLICATION_CREDENTIALS:-}"  # missing
```

See `docs/BLOCKED-live-compound-exhibit-2026-09-04.md`. Offline compound receipt authoritative.
...
- **Hosted `/stats` n drifts during the night** (301→309 while measuring). Pack uses **300+** lower bound; STATUS stamps a point-in-time n=306. AC8 tolerance widened to ±25 for that reason — a freeze of `/stats` was not available.
...
- **Did not run `full_gate.sh` end-to-end** — long_run + stranger trial hit hosted; offline subset + artifact gate verified instead.
