...
# free-tier CELEX (no Parallel): after seed_document_cache / boot_registry
python3 scripts/seed_document_cache.py && python3 -m clearance lookup "2012/28/EU"
```

Offline compound receipt writes `docs/COMPOUND-EXHIBIT-2026-08-29.md` with A=2→B=1 Parallel, corpus_hits≥1 — no Gemini/Parallel keys required. Overlapping claims in compound-mini A/B are **exact assertions** (shipping corpus identity); paraphrase reuse is intentionally refused.
...
| Public repo | Stranger can clone | [x] | **Public** since 2026-08-22 — verified `gh api repos/Morkeeth/agent-science --jq .private` → `false` (2026-09-15) |
...
| ADK default path | `engine_default: adk` | [x] local / hosted private | Local controls green; hosted now `mode=private-workspaces` (no public `/search`) |

**Controls re-measured 2026-09-15** (run each at object):
...
| artifact claims | `python3 scripts/eval_artifact_claims.py` | pack claims re-derived at HEAD (baseline arm trusts pack) |

**Compound exhibit (offline, 2026-09-15):** `python3 scripts/compound_exhibit_receipt.py` · A=**2**→B=**1** Parallel · B corpus hits=**2** — `docs/COMPOUND-EXHIBIT-2026-08-29.md`. Live hosted compound: **BLOCKED** this VM (no Parallel/Gemini keys) and public `/clear` is not exposed on current revision (`private-workspaces`). Orphan-works full script: prior **504** — do not claim on video.

**Eval gate:** `docs/QWEN-EVAL-GATE-2026-08-30.md` — baseline **5/6 = 0.833** vs shipping **6/6 = 1.000**, delta +1 (RC5); McNemar p=1.0000 at n=6. Artifact-claim gate: `docs/RECEIPT-night-wave-2026-09-15.md`.

**Local shelf (after `boot_registry.py`, 2026-09-15):** **241** claims in `cache/refusal_log.db` — re-derive; do not carry older "265+" hosted figures (hosted `/truths/ui` now requires workspace login).
...
**Offline receipt (2026-09-15):** `orphan-works` · fixtures `compound-mini-A.txt` → `compound-mini-B.txt` (exact overlapping assertions) · A=**2** Parallel · B=**1** Parallel · B corpus hits=**2** — `scripts/compound_exhibit_receipt.py`.

**Seal when:** hosted orphan-works A/B on `documentary-orphan-works*.txt` with durable GCS shelf (slice 1 deploy). Until then: draft only. **Note 2026-09-15:** hosted revision serves private workspaces only; sealed prediction assumes a public `/clear` path that is currently local-only.
...
**Try it (local, no key):** `python3 -m clearance lookup "2012/28/EU"` after `scripts/seed_document_cache.py`  
**Hosted:** https://agent-science-568004190078.us-central1.run.app — private research workspaces (access key); CLI/MCP is the primary stranger path  
...
- [x] Repo public (since 2026-08-22) — no flip needed
...
- [ ] Seal prediction hash in Devpost / commit message after live A/B (note: hosted `/clear` currently private)
- [ ] `bash deploy.sh` — candidate revision only; see `docs/DEPLOY-PREP-2026-09-15.md`
