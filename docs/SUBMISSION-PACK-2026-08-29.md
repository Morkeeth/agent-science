...
**Date:** 2026-09-19 · **Repo:** https://github.com/Morkeeth/agent-science @ `main` (visibility **public** since 2026-08-22)  
...
# includes: 72/72 · ADK path · partners · docs gate · holdout · offline compound ·
...
**Dictionary after cold `boot_registry.py` (2026-09-19):** **239** claims (25 sourced · 214 refused) — do not carry the older **265** figure; re-run boot to re-derive.
...
**Offline receipt (2026-08-29 / re-run 2026-09-19):** `orphan-works` · fixtures `compound-mini-A.txt` → `compound-mini-B.txt` · A=**2** Parallel · B=**1** Parallel · B corpus hits=**2** — `scripts/compound_exhibit_receipt.py`.
...
**Truths dashboard:** `/truths/ui` · re-derive claim count with `python3 scripts/boot_registry.py` then `python3 -c "from clearance.dictionary import economics; print(economics()['n'])"` (2026-09-19 cold boot → **239**)
...
**Always with the denominator.** 94% of *these 600 items*, measured. Never "94% of film archives" — see below for why that distinction is the demo.
...
- [x] Repo is public (since 2026-08-22) — no flip needed
...
- [ ] `bash deploy.sh` — restore public `/health` partner fields + promote candidate (see `docs/DEPLOY-PREP-2026-09-19.md`)
- [ ] Drop Parallel/Gemini console invoice into `fixtures/billing/invoice.json` so cost gate billing goes GREEN
