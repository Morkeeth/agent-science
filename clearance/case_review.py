...
def index(*, db=None, root=None, query='', review_only=False, limit=20, offset=0, include_cases=False):
    ...
    for cid in ids:
        ...
        if review_only and not affected:
            continue
        if matched >= offset:
            if include_cases:
...
def render(result, *, review_only=False, db=None):
    ...
    rows=result['cases']
    if not rows:
        return ('No decisions require review in the matching saved cases.' if review_only else 'No matching saved cases.')+'\n'+result['basis']+'\n'
    lines=[('DECISIONS TO REVIEW' if review_only else 'SAVED CASES'),result['basis'],'']
    for row in rows:
        lines.append(f"{row['id']} · v{row['version']} · {row['question']}")
        lines.append(f"  {row['review_required']} decisions need review · {row['evidence_count']} sources · {row['experiment_count']} experiments")
