...
def _anchor(data, item):
    ...
    if (not evidence or evidence.get('status') == 'UNAVAILABLE' or not evidence.get('snapshot_hash')
...
def _finding(data, finding, version):
    ...
    if relation != 'unresolved':
        ...
        if any(n not in anchor['quote'] for n in numbers):
...
def apply(case_id, version, proposal, *, db=None):
    ...
    with closing(cases.connect(db)) as con, con:
        ...
        for statement, assessment in prepared:
            claim = next((c for c in claims if c['statement'] == statement), None)
            if claim is None:
...
def build(case_data):
    ...
    for claim in brief['claims']:
        for assessment in claim['assessments']:
            ...
            for condition in conditions:
                ...
                current = source.get('snapshot_hash') == anchor['snapshot_hash'] and source.get('status') != 'UNAVAILABLE'
    ...
    return {'case_id':case_data['id'], 'version':case_data['version'], 'question':case_data['question'],
        ...
        'local_measurements':copy.deepcopy(case_data.get('experiments', [])),
...
def compare(case_id, from_version, *, db=None):
    ...
    return {'case_id':case_id, 'from_version':old['version'], 'version':new['version'],
        ...
        'affected_decisions':[d for d in new['decisions'] if d.get('review', {}).get('state') == 'REVIEW_REQUIRED'],
