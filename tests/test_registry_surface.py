...
def test_every_query_becomes_a_browsable_row():
    ...
    res = L.search_registry(con, "The Dust Bowl ruined forty million acres")
    after = L.browse_queries(con)
    assert len(after) == before + 1
    assert after[0]["query_text"] == "The Dust Bowl ruined forty million acres"
...
def test_sourced_returns_verbatim_span():
    ...
    res = L.search_registry(con, "The Act came into force on 1 April 2024")
...
def test_unknown_carries_named_refusal():
    ...
    res = L.search_registry(con, "Copyright was never evaluated for this item")
...
def test_the_separation_note_survives_a_non_zero_reuse_counter():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        for _ in range(3):
            L.search_registry(con, "The Act came into force on 1 April 2024")
